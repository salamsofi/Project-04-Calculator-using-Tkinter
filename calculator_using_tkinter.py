...
def getHistory():
    ...
    if history:
        # label_result.config(text=history)
# or
        label_result.config(text="\n".join(history[-3:]))  # Show last 3 entries
    else:
        label_result.config(text="No history yet")
...
def calculator():
    ...
    if equation != "":
        try:
            ...
            history.append(f"{equation} = Error (Divide by zero)")
            equation = ""
        except:
            result = "Error"
            history.append(f"{equation} - {result}")
...
label_result = Label(root, width=25, height=2, text="", font=("arial", 30), fg="#fff", bg="#17161b")
label_result.pack()

Button(root, text="C", width=5, height=1, font= ("arial",30,"bold"), bd=1, fg="#fff", bg="#3697f5", command = lambda: clear()).place(x=10, y=100)
Button(root, text="CE", width=5, height=1, font= ("arial",30,"bold"), bd=1, fg="#fff", bg="#2a2d36", command = lambda: backspace()).place(x=150, y=100)
Button(root, text="H", width=5, height=1, font= ("arial",30,"bold"), bd=1, fg="#fff", bg="#fe9037", command = lambda: getHistory()).place(x=290, y=100)
...
Button(root, text="=", width=5, height=1, font= ("arial",30,"bold"), bd=1, fg="#fff", bg="#fe9037", command = lambda: calculator()).place(x=290, y=500)
Button(root, text="+", width=5, height=1, font= ("arial",30,"bold"), bd=1, fg="#fff", bg="#2a2d36", command = lambda: show("+")).place(x=430, y=500)
